...
public class XML implements Serializable {
	...
	public XML(File file, String options) throws IOException, ParserConfigurationException, SAXException {
		this(IOUtil.createReader(file), options);
	}
	...
	public boolean save(File file, String options) {
		PrintWriter writer = IOUtil.createWriter(file);
	...
	public XML[] getChildren(String name) {
		...
		if (name.indexOf('/') != -1) {
			return getChildrenRecursive(IOUtil.split(name, '/'), 0);
		}
		// if it's a number, do an index instead
		...
		return (XML[]) IOUtil.subset(matches, 0, matchCount);
	}
	...
	protected XML[] getChildrenRecursive(String[] items, int offset) {
		...
		for (int i = 0; i < matches.length; i++) {
			...
			outgoing = (XML[]) IOUtil.concat(outgoing, kidMatches);
		}
		return outgoing;
	}
	...
	protected XML appendChild(Node newNode) {
		...
		if (children != null) {
			children = (XML[]) IOUtil.concat(children, new XML[] { newbie });
		}
		return newbie;
	}
	...
	public int getIntContent(int defaultValue) {
		return IOUtil.parseInt(node.getTextContent(), defaultValue);
	}
	...
	public float getFloatContent(float defaultValue) {
		return IOUtil.parseFloat(node.getTextContent(), defaultValue);
	}
	...
	public String format(int indent) {
		try {
			...
			String[] tempLines = IOUtil.split(tempWriter.toString(), sep);
			...
			if (tempLines[0].startsWith("<?xml")) {
				...
				if (tempLines[0].length() == declEnd) {
					...
					tempLines = IOUtil.subset(tempLines, 1);
				} else {
			...
			String singleLine = IOUtil.join(IOUtil.trim(tempLines), "");
			if (indent == -1) {
				return singleLine;
			}
